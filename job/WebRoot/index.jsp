...
<body>
	<div id="body">
		...
		<div id="container">
			...
			<div class="content">
				<div id="search_box">
					<form id="searchForm" action="<%=basePath %>work/list" method="post">
						<ul id="searchType">
...
.ui-menu-item a {
	...
	overflow: hidden;
				...
				<ul class="reset hotabbing">
					<c:if test="${sessionScope.index_user!=null && sessionScope.index_user.id!=null && sessionScope.index_user.type==3}">
				...
				<div id="hotList">
					<c:if test="${sessionScope.index_user!=null && sessionScope.index_user.id!=null && sessionScope.index_user.type==3}">
						<ul class="hot_pos reset">
							<c:forEach items="${recomWorks }" var="work">
								<li class="clearfix">
									<div class="hot_pos_l">
										<div class="mb10">
											<a href="<%=basePath %>work/info?workId=${work.id }" target="_blank">${work.name }</a> &nbsp; <span class="c9">[${work.city }]</span>
										</div>
										<span><em class="c7">月薪： </em>${work.salaryMin }k-${work.salaryMax }k</span> <span><em
											class="c7">经验：</em> ${work.workExp }</span> <span><em class="c7">最低学历：
										</em>${work.education }</span> <br /> <span><em class="c7">职位诱惑：</em>${work.advantage }</span> <br /> <span>${work.createDate }</span>
										<!-- <a  class="wb">分享到微博</a> -->
									</div>
								</li>
							</c:forEach>
							<li>
								<a href="work/list" class="btn fr" target="_blank">查看更多</a>
							</li>
						</ul>
 ...
 -->
			...
			<!------------------------------------- end ----------------------------------------->
			<script type="text/javascript" src="theme/default/js/Chart.min.js"></script>
			<script type="text/javascript" src="theme/default/js/home.min.js"></script>
	...
	<script type="text/javascript">
		$(".menu_box").find("a").each(function(){
			$(this).attr("href","javascript:;");
		});
		$(".menu_box").find("a").click(function(){
